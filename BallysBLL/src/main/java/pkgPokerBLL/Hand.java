...
public class Hand {
	...
	public static ArrayList<Hand> ExplodeHands(Hand h) {
		...
		for (Card c : h.getCardsInHand()) {
			if ((c.getWild() == true) || (c.geteRank() == eRank.JOKER)) {
				int i = h.getCardsInHand().indexOf(c);
				ReturnHands = ReplaceCard(ReturnHands, i);
			}
		}
	...
	public static boolean isHandFiveOfAKind(Hand h, HandScore hs) {
        ...
        if(isHandFourOfAKind(h,hs)){
            ...
            if(h.getCardsInHand().get(eCardNo.SecondCard.getCardNo()).geteRank() == eRank.JOKER){
                isFiveOfAKind = true;
                hs.setHandStrength(eHandStrength.FiveOfAKind);
